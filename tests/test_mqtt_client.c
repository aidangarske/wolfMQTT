 ...
 * different method (or omitting it) must be refused before anything reaches the
 * wire, so an application cannot switch mechanisms mid-session. */
TEST(auth_mismatched_method_rejected)
{
    ...
    /* An AUTH omitting the Authentication Method entirely is likewise refused. */
    XMEMSET(&auth, 0, sizeof(auth));
    auth.reason_code = MQTT_REASON_CONT_AUTH;
    g_frames_written = 0;
